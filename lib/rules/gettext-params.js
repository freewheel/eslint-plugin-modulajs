...
module.exports.meta = {
  docs: {
    description:
      'allow only string/template literals in gettext family functions',
...
module.exports = function(context) {
  ...
  function isGettextCall(node) {
    return Boolean(
      node.callee.type === 'Identifier' &&
        gettextFamily.includes(node.callee.name)
    );
  }
  ...
  */
  function hasArguments(node, n) {
    return Boolean(node.arguments && node.arguments.length > (n || 0));
  }
  ...
  */
  function hasExpression(node) {
    return Boolean(node.expressions && node.expressions.length > 0);
  }
  ...
  */
  function isValidTemplate(node) {
    return Boolean(isTemplateLiteral(node) && !hasExpression(node));
  }
  ...
  function isValidBinaryExpression(node) {
    return Boolean(
      node.type === 'BinaryExpression' &&
        node.operator === '+' &&
        (isSupportedNode(node.right) && isSupportedNode(node.left))
    );
  }
  ...
  function validateGettext(node) {
    if (hasArguments(node)) {
      if (
        ...
        hasExpression(node.arguments[0])
      ) {
        context.report({
          node,
          message:
            'Template Literals cannot contain expressions inside gettext family functions'
        });
      } else if (!isSupportedNode(node.arguments[0])) {
        context.report({
          node,
          message:
            'First argument of Gettext functions must be String or Template Literals'
        });
      }
    } else {
      context.report({
  ...
  function validateNgettext(node) {
    if (hasArguments(node, 1)) {
      if (
        (isTemplateLiteral(node.arguments[0]) &&
          hasExpression(node.arguments[0])) ||
        (isTemplateLiteral(node.arguments[1]) &&
          hasExpression(node.arguments[1]))
      ) {
        context.report({
          node,
          message:
        ...
        context.report({
          node,
          message:
            'First two arguments of Ngettext functions must be String or Template Literals'
        });
      }
    } else {
      context.report({
    ...
    if (hasArguments(node, 1)) {
      if (
        ...
        context.report({
          node,
          message:
            'First two arguments of Dgettext functions must be String or Template Literals'
        });
      }
    } else {
      context.report({
  ...
  function validateDngettext(node) {
    if (hasArguments(node, 2)) {
      if (
        ...
        (isTemplateLiteral(node.arguments[1]) &&
          hasExpression(node.arguments[1])) ||
        (isTemplateLiteral(node.arguments[2]) &&
          hasExpression(node.arguments[2]))
      ) {
        context.report({
          node,
          message:
        ...
        context.report({
          node,
          message:
            'First three arguments of Dngettext functions must be String or Template Literals'
        });
      }
    } else {
      context.report({
