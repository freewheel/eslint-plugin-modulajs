...
module.exports.meta = {
  docs: {
    description: 'Use function to define default values for non-primitive props in Modula model',
...
/**
 * Checks if node is an instance of the Modula Model class
 * @param {ASTNode} node The AST node being checked
 * @returns {boolean} True is node is a Model, false is not
 */
function isCreateModel(node) {
  return Boolean (
    node.callee && node.callee.name === 'createModel'
  );
}
...
function isPermittedValue(node) {
  return (
    node.type === 'Literal' ||
    (node.type === 'UnaryExpression' && node.operator === '-' && node.prefix &&
      node.argument && node.argument.type === 'Literal' && !isNaN(node.argument.value)) || // negative number
...
function checkDefaults(declarations, context) {
  declarations.forEach(function(declaration) {
    if (declaration.type === 'Property' && !isPermittedValue(declaration.value)) {
      context.report({
        node: declaration,
        message: 'Please use function to define non-primitive default values in Model'
      });
    }
  });
}
