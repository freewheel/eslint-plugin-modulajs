 ...
 */
const isNotAscending = (array, key) => array.some((val, idx, arr) => idx && val[key] < arr[idx - 1][key]);
...
module.exports = function createModelAttrsOrder(context) {
  ...
  function propsToProps(parentNode) {
    ...
    if (args.length > 0 && args[0].properties && args[0].properties.filter) {
      const props = args[0].properties.filter(node => node.key && node.key.name);
  ...
  function validateLifeCyclePropNames(props) {
    const lifeCycleOrder = options.lifeCycleProps.reduce((order, propName, idx) => {
      order[propName] = idx + 1;
      return order;
    }, {});

    const { props: lifeCycleProps, notHandled } = props.reduce((memo, prop) => {
      const { name } = prop;
    ...
    if (isNotAscending(lifeCycleProps, 'order')) {
      const usedProps = lifeCycleProps.reduce((memo, prop) => Object.assign(memo, { [prop.name]: true }), {});

      context.report({
        node: lifeCycleProps[0].node,
        message: 'Lifecycle props used out of order: found {{ used }}. Expected {{ correct }}',
        data: {
          used: lifeCycleProps.map(prop => prop.name).join(', '),
          correct: options.lifeCycleProps.filter(propName => usedProps[propName]).join(', ')
        }
      });
    }

    return {
      lastLifeCycleProp: lifeCycleProps.length > 0 ? lifeCycleProps.reverse()[0] : { index: -1, name: '' },
      notHandled
    };
  }
  ...
  function validateSendRecvPropNames(props, lastLifeCycleProp) {
    ...
    return props.reduce((memo, prop, idx) => {
      ...
      if (isRecv || sendRE.test(name)) {
        ...
        if (prop.index < lastLifeCycleProp.index) {
          context.report({
            node,
            message: '{{ propName }} should come after {{ lifeCyclePropName }}',
            data: { propName: name, lifeCyclePropName: lastLifeCycleProp.name }
          });
        }
        ...
        if (options.sendRecvPairs && isRecv) {
          ...
          if (prevPropName !== senderName) {
            context.report({
              node,
              message: '{{ propName }} should follow {{ senderName }}, not {{ prevPropName }}',
  ...
  function validateHelperPropNames(props, lastLifeCycleProp, lastSendRecvProp) {
    props.forEach(prop => {
      ...
      const isAfterLastSendRecv = lastSendRecvProp && index < lastSendRecvProp.index;
      ...
      if (isAfterLastSendRecv || index < lastLifeCycleProp.index) {
        context.report({
          ...
          data: {
            propName: name,
            last: (isAfterLastSendRecv ? lastSendRecvProp : lastLifeCycleProp).name
          }
        });
      }
    });
    ...
    CallExpression: function(node) {
      if (isCreateModelCall(node)) {
        ...
        if (props.length > 0) {
          ...
          const {
            lastLifeCycleProp,
            notHandled: notHandledByLifeCycle
          } = validateLifeCyclePropNames(props);

          const {
            lastSendRecvProp,
            notHandled: notHandledBySendRecv
          } = validateSendRecvPropNames(
            notHandledByLifeCycle,
            lastLifeCycleProp
          );
