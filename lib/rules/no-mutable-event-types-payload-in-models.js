...
module.exports.meta = {
  docs: {
    description: 'disallow mutable objects in eventTypes/watchEventTypes payload defination',
  ...
  return {
    ...
    ObjectExpression: function(node) {
      ...
      node.properties.forEach(function(property) {
        if (isTargetDeclaration(property.key, 'eventTypes') || isTargetDeclaration(property.key, 'watchEventTypes')) {
          isEventTypesFlag = true;
        }

        if (isEventTypesFlag && isTargetDeclaration(property.key, 'payload')) {
          checkForbidden(property.value.properties, context, isModelFlag, getMessage);
        }
      });
    },
